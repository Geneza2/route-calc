...
type MapProps = {
  ...
  /** Prefer Latin labels when available in the basemap. */
  useLatinLabels?: boolean;
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  { children, styles, projection, useLatinLabels = true, ...props },
  ref
) {
  ...
  const applyLatinLabels = useCallback((map: MapLibreGL.Map) => {
    ...
    style.layers.forEach((layer) => {
      ...
      if (!shouldOverrideTextField(textField)) return;
      if (!map.getLayer(layer.id)) return;
      map.setLayoutProperty(layer.id, "text-field", latinTextField);
    });
  }, []);
